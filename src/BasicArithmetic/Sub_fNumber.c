 ...
 * Subtracts a number from an array and saves the result in 'Sub_fNumber_out.txt'.
 */
...
void main(int argc, char *argv[]) {
	...
	if(argc != 3) {
		...
		printf("\n Sub_fArray.exe 'filename_array1' 'integer'");
		printf("\n\n");
		exit(EXIT_FAILURE);
	}
	char *file1 = argv[1];
	...
	StoreFloatInFile("Sub_fNumber_out.txt", nDataResult, dataResult);
